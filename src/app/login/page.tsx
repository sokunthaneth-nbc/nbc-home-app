...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const { valid, errors } = validateForm(staffId, password);
    setErrors(errors);
    ...
    try {
      ...
      if (userStatusCode === 1) {
        if (invalidCredentials === "Invalid email or password") {
          setErrors({
            staffId: "ពត៍មានមិនត្រឹមត្រូវ",
            password: "ពត៍មានមិនត្រឹមត្រូវ",
          });
        }
        return;
      }
  ...
  useLayoutEffect(() => {
    const biometricState = localStorage.getItem("biometric-enabled") === "true";
    setIsBiometricEnabled(biometricState);
  }, []);
    ...
    <section className="bg-white dark:bg-[#001346] min-h-screen flex flex-col justify-between">
      <div className="w-full  px-6 py-8 flex-1 overflow-auto">
        ...
        <form
          ...
          //   onSubmit={hasTyped ? handleLogin : handleVerifyPasskey}
          //   onSubmit={
          //     isBiometricEnabled && !hasTyped ? handleVerifyPasskey : handleLogin
          //   }
          onSubmit={(e) =>
            isBiometricEnabled && !hasTyped && !biometricFailed
        ...
        >
          ...
          <div>
            <label
              htmlFor="staffId"
              className="block mb-2 text-sm font-normal text-[#040e28] dark:text-white"
            >
